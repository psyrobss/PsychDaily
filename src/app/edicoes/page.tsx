...
export default function EdicoesPage() {
  ...
  const allEdicoes = useMemo(() => {
    return [...todasEdicoes].sort((a, b) => parseInt(b.edicao) - parseInt(a.edicao));
  }, []);

  const totalPages = useMemo(() => {
    return Math.ceil(allEdicoes.length / ITEMS_PER_PAGE);
  }, [allEdicoes]);

  const currentEdicoes = useMemo(() => {
    const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
    const endIndex = startIndex + ITEMS_PER_PAGE;
    return allEdicoes.slice(startIndex, endIndex);
  }, [currentPage, allEdicoes]);
